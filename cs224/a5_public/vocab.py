...
class VocabEntry(object):
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        sent_chars = self.words2charindices(sents)
        sent_chars_padded = pad_sents_char(sent_chars, self.char_pad)
        # print("sent_chars_padded:", len(sent_chars_padded), max(map(len, sent_chars_padded)), sent_chars_padded[0])
        # print("size of padded sent chars:", torch.tensor(sent_chars_padded, dtype=torch.long, device=device).size())
        sents_var = torch.tensor(sent_chars_padded, dtype=torch.long, device=device).transpose(0, 1).contiguous()
        return sents_var
        ### END YOUR CODE
